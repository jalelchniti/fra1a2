...
interface SchoolPrompt {
  ...
  color: string;
  emoji: string;
  mood: 'positive' | 'challenging' | 'reflective';
}

const SchoolMemoriesGame: FC = () => {
  const [currentCard, setCurrentCard] = useState<number>(0);
  const [score, setScore] = useState<number>(0);
  const [usedCards, setUsedCards] = useState<number[]>([]);
  const [showFeedback, setShowFeedback] = useState<boolean>(false);

  const schoolPrompts: SchoolPrompt[] = [
    {
      id: 1,
  ...
  const getMoodColor = (mood: 'positive' | 'challenging' | 'reflective'): string => {
    switch(mood) {
